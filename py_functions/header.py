...
Res = [0.15,0.01, 0.05,0.1,0.15,0.5,1.0,5.0,10.0]
S_start2 ={}
for Chi in Chis:
    S_start2[str(Chi)] = {}
    for Theta in Thetas:
...
Res = S_start2[str(Chi)][str(Theta)].parameters_list[1:]#
S2 = {}
for Chi in Chis:
    S2[str(Chi)] = {}
    for Re in Res:
        ...
        S2[str(Chi)][str(S_temp_para)]['F_ad'] = []
    ...
    for Theta in Thetas:
        for Re in Res:
            ...
            S_temp_F_ad  =S_start2[str(Chi)][str(Theta)].F_ad[i]
            ...
            S2[str(Chi)][str(S_temp_para)]['F_ad'].append(S_temp_F_ad)

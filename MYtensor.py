...
class TensAdd(TensExpr, AssocOp):
    ...
    def contract_X(self, x):
        args = [y.contract_X(x) for y in self.args]
        t = TensAdd(*args)
        return t
    def int_X(self, x):
...
def simplify_R(expr):
    if expr.args:
        ...
        if isinstance(expr,Add):
            ...
            # print([arg for arg in expr.args ])
    # print([arg for arg in expr.args if  isinstance(arg,R)])
...
class R(Symbol):
    def __new__(cls,N,Xo = 0):
        if N==0:
            return 1
        if Xo:
            obj = Symbol.__new__(cls,f'r^{N}_{Xo}' if N != 1 else 'r')
        else: 

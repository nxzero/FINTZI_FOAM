...
class TensAdd(TensExpr, AssocOp):
    ...
    def contract_X(self, x):
        g,X,result,rname = x
        if isinstance(X,TensorHead):
            args = [y.contract_X(x) if y.has(X) else (y.contract_delta(g) if isinstance(y,TensExpr)  else y) for y in self.args ]
        else:
            args = [y.contract_X(x) if (y.has(X[1]) and y.has(X[0])) else (y.contract_delta(g) if isinstance(y,TensExpr)  else y) for y in self.args ]
        t = TensAdd(*args)

        if t.get_indices():
            return t
        else: 
            return expand(Add(*[y for y in t.args]))    
    
    def int_X(self, x):
        args = [y.int_X(x) for y in self.args]
        t = TensAdd(*args)
        return t
    ...
    def _eval_partial_derivative(self, s):
        ...
        list_addends = []
        for a in self.args:
            if isinstance(a, (TensExpr,R)):
                list_addends.append(a._eval_partial_derivative(s))
            elif a.has(R):
                list_addends.append(D(a,s))
...
class TensMul(TensExpr, AssocOp):
    ...
    def int_X(self,x):
        g,X = x
        expr = self.contract_metric(g)
        if self != expr:
            return int_X(expr, x)
        
        # list of positions of the ``X`` inside ``args``
        gidx = [ y.indices[0] for i, y in enumerate(expr.args) if isinstance(y, Tensor) and y.component == X]
        gpos = [ i for i, y in enumerate(expr.args) if isinstance(y, Tensor) and y.component == X]
        # print(gidx)
        # if we integrate any odd number of x then return 0 
        if not gpos:
            return expr * 4
        if len(gpos) % 2:
            return 0
        
        factor = Mul(*[i+3 for i in range(len(gidx))[2::2]])*3/4
        exprI = generate_perm(gidx,g)/factor
        expr = TensMul(*[t for i,t in enumerate(expr.args) if i not in gpos])
        return  (exprI*expr)
    ...
    def contract_X(self,x):
        ...
        if self != expr:
            return contract_X(expr, x)
        
        ## contract the Hrmc func
        expr = TensMul(*[contract_X(y,x) for y in expr.args])
        
        # list of positions of the ``X`` inside ``args``
        if isinstance(X,TensorHead):
            gpos = [(i, *y.indices) for i, y in enumerate(self.args) if isinstance(y, Tensor) and y.component == X]
        else:
            gpos = [(i, *y.indices) for i, y in enumerate(self.args) if isinstance(y, Tensor) and (y.component == X[0] or y.component == X[1])]
            

        # position of the pair of x args 
        contracts=[]
        for i,idxvalue in enumerate(gpos):
            for j,idx2value2 in enumerate(gpos[i:]):
                idx,value = idxvalue
                idx2,value2 = idx2value2
                if value == -value2:
                    if self.args[idx].component != self.args[idx2].component:
                        contracts.append(idx)
                        contracts.append(idx2)
                    elif isinstance(X,TensorHead):
                        contracts.append(idx)
                        contracts.append(idx2)
                        
                    

        # replacement of X*X by r^2 Or 1 if the option unit is passed
        if not result:
            r = R(len(contracts),rname)
            expr = TensMul(*[t for i,t in enumerate(expr.args) if i not in contracts])*r
        else: 
            expr = TensMul(*[t for i,t in enumerate(expr.args) if i not in contracts])* (result)**Rational(len(contracts)/2)        
        return expr
...
def contract_X(t, x):
    if isinstance(t, TensExpr):
        return t.contract_X(x)
    return t

def int_X(t, x):
    if isinstance(t, (TensExpr,Tensor)):
        return t.int_X(x)
    return t
...
class R(Symbol):
    def __new__(cls,N,Xo=0):
        if N==0:
            return 1
        if Xo:
            obj = Symbol.__new__(cls,f'r^{N}_{Xo}' if N != 1 else 'r_{Xo}')
        else: 
            obj = Symbol.__new__(cls,f'r^{N}' if N != 1 else 'r')
        obj.N = N
        obj.Xo = Xo
        return obj
    ...
    def _eval_partial_derivative(self,v):
        if isinstance(v, TensExpr):
            ...
            return (self.N * v * R(self.N-2,self.Xo)).doit()
        else:
            return 0
    ...
    def contract_metric(self,g):
        return self
    def contract_delta(self,g):
        return self
    
    def __mul__(self, other):
        if isinstance(other, R):
            if other.Xo == self.Xo:
                return R(self.N+ other.N,self.Xo)
        if isinstance(other, TensExpr):
    ...
    def __rmul__(self, other):
        if isinstance(other, R):
            if other.Xo == self.Xo:
                return R(self.N+ other.N,self.Xo)
        if isinstance(other, (TensExpr,Integer)):
            return TensMul(self,other)
        else:
            return Mul(self,other)
        
    def __pow__(self, exponent):
        return R(self.N*exponent,self.Xo)

def D(expr, v):
    if isinstance(expr, TensExpr):
        return expr._eval_partial_derivative(v)
    if isinstance(expr, R):
        return expr._eval_partial_derivative(v)
    elif isinstance(expr, Add):
        return Add(*[D(arg, v) for arg in expr.args if arg.has(R)])
        ...
        for i, factor in enumerate(expr.args):
            if factor.has(R):
                ...
                terms.append(Mul(d_factor, *other_factors))
        return Add(*terms)
    elif isinstance(expr,Pow):
        if isinstance(expr.args[0],R):
            if expr.args[1].has(R,TensExpr):
                return expr.args[0]**expr.args[1]*(
                    D(expr.args[1],v)*log(expr.args[0])
                    + D(expr.args[0],v)*expr.args[1]/expr.args[0]
                )
            else:
                return expr.args[0]**(expr.args[1]-1)*D(expr.args[0],v)*expr.args[1]
...
def generate_perm(ids,g):
    ...
    for i,ix in enumerate(ids[1:]):
        ...
        if len(remaining_id) >= 2:
            expr = generate_perm(remaining_id,g) 
        terms.append(g(i1,ix) * expr)

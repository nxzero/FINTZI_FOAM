...
class AP():
    def __init__(self,para_name: str,parameters_list: list,namestud: str):
        ...
        self.Nb = 100              # number of bubbles (Must be a perfect square or it will be rounded) 
        # numerical parameters 
        self.ndc = 44               # number of cells per diameters of a bubble
        ...
        # numerical parameters
        self.LEVEL = math.ceil(math.log(self.ndc*self.Ls/self.D)/math.log(2.)) #LEVEL of refinement of the mesh Might change acctually 
    ...
    def calcul_parameters(self):
        if self.parameters['dimension'] == 2:                                                 
            self.N = round(math.sqrt(self.parameters['Nb']))                          
            self.Ls = self.N*self.parameters['D']*math.sqrt(math.pi/(4.*self.parameters['PHI']))   
        if self.parameters['dimension'] == 3:                                                  
            self.N = round(math.pow(self.parameters['Nb'],1./3.))               
            self.Ls = self.N*self.parameters['D']*math.pow(math.pi/(6.*self.parameters['PHI']),1./3.)   # size of the domain
        ...
        # numerical parameters
        self.LEVEL = math.ceil(math.log(self.parameters['ndc']*self.Ls/self.parameters['D'])/math.log(2.))  #LEVEL of refinement of the mesh Might change acctually 

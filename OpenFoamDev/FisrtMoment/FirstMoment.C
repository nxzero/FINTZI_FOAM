...
{
    defineTypeNameAndDebug(FirstMoment, 0);
    addToRunTimeSelectionTable(functionObject, FirstMoment, dictionary);
}
}
...
Foam::word Foam::functionObjects::FirstMoment::fieldName(const word& name) const
{
    return this->name() + ":" + name;
}


void Foam::functionObjects::FirstMoment::createFiles()
{
    ...
    if (writeToFile() && !forceFilePtr_)
    {
        ...
        firstmomentFilePtr_ = createFile("firstmoment");
        writeIntegratedHeader("firstmoment", firstmomentFilePtr_());
        ...
        if (nBin_ > 1)
        {
            ...
            firstmomentFilePtr_ = createFile("firstmomentBin");
            writeBinHeader("firstmoment", firstmomentFilePtr_());
        }
    }
}


void Foam::functionObjects::FirstMoment::writeIntegratedHeader
(
    const word& header,
    Ostream& os
) const
...
void Foam::functionObjects::FirstMoment::writeBinHeader
(
    const word& header,
    Ostream& os
) const
...
void Foam::functionObjects::FirstMoment::setCoordinateSystem
(
...
void Foam::functionObjects::FirstMoment::initialise()
{
    if (initialised_)
    {
        return;
...
void Foam::functionObjects::FirstMoment::initialiseBins()
{
    if (nBin_ > 1)
    {
    ...
    // Allocate storage for FirstMoment and moments
    forAll(force_, i)
    {
        force_[i].setSize(nBin_, vector::zero);
        // // moment_[i].setSize(nBin_, vector::zero);
        firstmoment_[i].setSize(nBin_, tensor::zero);
    }
}


void Foam::functionObjects::FirstMoment::resetFields()
{
    force_[0] = Zero;
    force_[1] = Zero;
    force_[2] = Zero;
    ...
    if (writeFields_)
    {
        ...
        firstmoment == dimensionedTensor(firstmoment.dimensions(), Zero);
    }
}


Foam::tmp<Foam::volSymmTensorField>
Foam::functionObjects::FirstMoment::devRhoReff() const
{
...
Foam::tmp<Foam::volScalarField> Foam::functionObjects::FirstMoment::mu() const
{
...
Foam::tmp<Foam::volScalarField> Foam::functionObjects::FirstMoment::rho() const
{
    if (rhoName_ == "rhoInf")
    {
...
Foam::scalar Foam::functionObjects::FirstMoment::rho(const volScalarField& p) const
{
...
void Foam::functionObjects::FirstMoment::applyBins
(
    const vectorField& Md,
    const vectorField& fN,
    const vectorField& fT,
    const vectorField& fP,
    const vectorField& d   //center of cells in the patch 
)
{
    if (nBin_ == 1)
    {
        ...
        firstmoment_[0][0] += sum(Md * fN);
        firstmoment_[1][0] += sum(Md * fT);
        firstmoment_[2][0] += sum(Md * fP);
    }
    else
    {
        ...
        forAll(dd, i)
        {
            ...
            firstmoment_[0][bini] += Md[i] * fN[i];
            firstmoment_[1][bini] += Md[i] * fT[i];
            firstmoment_[2][bini] += Md[i] * fP[i];
        }
    }
}


void Foam::functionObjects::FirstMoment::addToFields
(
    const label patchi,
    const vectorField& Md,//dist to orig 
    const vectorField& fN,//force Normal due to pressure 
    const vectorField& fT,//force Teng due to viscosity 
    const vectorField& fP// porous force 
)
{
    if (!writeFields_)
    {
    ...
    auto& firstmoment = lookupObjectRef<volTensorField>(fieldName("firstmoment"));
    tensorField& pfm = firstmoment.boundaryFieldRef()[patchi];
    pfm = Md * pf;
}
...
)
{
    ...
    forAll(cellIDs, i)
    {
        ...
        firstmoment[celli] = Md[i] * force[celli];
    }
}


void Foam::functionObjects::FirstMoment::writeIntegratedForceMoment
(
...
(
    const string& descriptor,
    const tensorField& fm0,
    const tensorField& fm1,
    const tensorField& fm2,
    autoPtr<OFstream>& osPtr
) const
{
    tensor pressure = sum(fm0);
    tensor viscous = sum(fm1);
    tensor porous = sum(fm2);
    tensor total = pressure + viscous + porous;
...
void Foam::functionObjects::FirstMoment::writeFirstMoment()
{
...
void Foam::functionObjects::FirstMoment::writeBinnedForceMoment
(
...
) const
{
    ...
    forAll(f[0], i)
    {
        tensor total = f[0][i] + f[1][i] + f[2][i];
...
void Foam::functionObjects::FirstMoment::writeBins()
{
    List<Field<vector>> lf(3);
    List<Field<vector>> lm(3);
    List<Field<tensor>> ls(3);
    ...
    writeBinnedForceMoment(ls, firstmomentBinFilePtr_);
}


// * * * * * * * * * * * * * * * * Constructors  * * * * * * * * * * * * * * //

Foam::functionObjects::FirstMoment::FirstMoment
(
    const word& name,
    const Time& runTime,
    const dictionary& dict,
...
bool Foam::functionObjects::FirstMoment::read(const dictionary& dict)
{
    ...
    if (writeFields_)
    {
        ...
        (
            ...
            (
                ...
                mesh_,
                dimensionedTensor(dimForce*dimLength, Zero)
            )
        );

        mesh_.objectRegistry::store(firstmomentPtr);
    }
...
void Foam::functionObjects::FirstMoment::calcFirstMomentMoment()
{
    initialise();
    ...
    Pstream::listCombineGather(firstmoment_, plusEqOp<tensorField>());
    Pstream::listCombineScatter(force_);
    // Pstream::listCombineScatter(moment_);
    Pstream::listCombineScatter(firstmoment_);
}


Foam::vector Foam::functionObjects::FirstMoment::forceEff() const
{
    return sum(force_[0]) + sum(force_[1]) + sum(force_[2]);
}


// Foam::vector Foam::functionObjects::FirstMoment::momentEff() const
// {
    // // // return sum(moment_[0]) + sum(moment_[1]) + sum(moment_[2]);
// }

Foam::tensor Foam::functionObjects::FirstMoment::firstmomentEff() const
{
    return sum(firstmoment_[0]) + sum(firstmoment_[1]) + sum(firstmoment_[2]);
}


bool Foam::functionObjects::FirstMoment::execute()
{
    calcFirstMomentMoment();
    ...
    setResult("normalFirstMoment", sum(firstmoment_[0]));
    setResult("tangentialFirstMoment", sum(firstmoment_[1]));
    setResult("porousFirstMoment", sum(firstmoment_[2]));

    return true;
}


bool Foam::functionObjects::FirstMoment::write()
{
    if (writeFields_)
    {

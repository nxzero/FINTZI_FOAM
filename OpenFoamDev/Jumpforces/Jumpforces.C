...
{
    defineTypeNameAndDebug(Jumpforces, 0);
    addToRunTimeSelectionTable(functionObject, Jumpforces, dictionary);
}
}

// * * * * * * * * * * * * Protected Member Functions  * * * * * * * * * * * //

Foam::word Foam::functionObjects::Jumpforces::fieldName(const word& name) const
{
    return this->name() + ":" + name;
}


void Foam::functionObjects::Jumpforces::createFiles()
{
...
void Foam::functionObjects::Jumpforces::writeIntegratedHeader
(
    const word& header,
    Ostream& os
) const
...
void Foam::functionObjects::Jumpforces::writeBinHeader
(
    const word& header,
    Ostream& os
) const
...
void Foam::functionObjects::Jumpforces::setCoordinateSystem
(
...
void Foam::functionObjects::Jumpforces::initialise()
{
    if (initialised_)
    {
        return;
...
void Foam::functionObjects::Jumpforces::initialiseBins()
{
    if (nBin_ > 1)
    {
    ...
    // Allocate storage for Jumpforces and moments
    forAll(force_, i)
    {
...
void Foam::functionObjects::Jumpforces::resetFields()
{
    force_[0] = Zero;
    force_[1] = Zero;
    force_[2] = Zero;
...
Foam::functionObjects::Jumpforces::devRhoReff() const
{
...
Foam::tmp<Foam::volScalarField> Foam::functionObjects::Jumpforces::mu() const
{
...
Foam::tmp<Foam::volScalarField> Foam::functionObjects::Jumpforces::rho() const
{
    if (rhoName_ == "rhoInf")
    {
...
Foam::scalar Foam::functionObjects::Jumpforces::rho(const volScalarField& p) const
{
...
void Foam::functionObjects::Jumpforces::applyBins
(
...
void Foam::functionObjects::Jumpforces::addToFields
(
    const label patchi,
    const vectorField& Md,
    const vectorField& fN,
...
void Foam::functionObjects::Jumpforces::writeIntegratedForceMoment
(
...
void Foam::functionObjects::Jumpforces::writeForces()
{
...
void Foam::functionObjects::Jumpforces::writeBinnedForceMoment
(
...
void Foam::functionObjects::Jumpforces::writeBins()
{
...
Foam::functionObjects::Jumpforces::Jumpforces
(
    const word& name,
    const Time& runTime,
    const dictionary& dict,
...
bool Foam::functionObjects::Jumpforces::read(const dictionary& dict)
{
...
void Foam::functionObjects::Jumpforces::calcForcesMoment()
{
    initialise();
...
Foam::vector Foam::functionObjects::Jumpforces::forceEff() const
{
    return sum(force_[0]) + sum(force_[1]) + sum(force_[2]);
}


Foam::vector Foam::functionObjects::Jumpforces::momentEff() const
{
    return sum(moment_[0]) + sum(moment_[1]) + sum(moment_[2]);
}


bool Foam::functionObjects::Jumpforces::execute()
{
    //////////////////////////////////////:
    // const volVectorField & U = lookupObject<volVectorField>(UName_);
    // label patchID = mesh_.boundaryMesh().findPatchID(Patch1_);
    ...
    // 	)
    // );
    // dictionary& BCdict = solDict.subDict("boundaryField").subDict("frontWallCyclic");
    // scalar jumpTable2 = readScalar(BCdict.lookup("jumpTable2"));
    // Info<< "    !!!!!! JT2         : " << jumpTable2 << endl;
...
bool Foam::functionObjects::Jumpforces::write()
{
    if (writeFields_)
    {

...
from sympy.tensor.tensor import Tensor, TensExpr, TensAdd, TensMul, Expr, TensorSymmetry, R, D
...
class PartialDerivative(TensExpr):
    ...
    def _perform_derivative(self,g,g2):
        result = self.expr
        display(result,type(result),self.variables)

        for v in self.variables:
            if isinstance(result, (TensExpr,R)):
                ...
                if v._diff_wrt:
                    ...
                    result = S.Zero
        return result.contract_delta(g).contract_metric(g2)
